...
//-----------------------------------------------------------------------------
    /*!
    Sorts eigenpairs (lambda) with respect to a shift value (eta) such that:
    |lambda[0] - eta| < |lambda[1] - eta| < |lambda[2] - eta| < ...

    :Inputs:

        VectorXd& eigenvalues:
            Eigenvalues to be sorted (lambda)
...
//-----------------------------------------------------------------------------
    ...
    sort(indices.begin(), indices.end(), [&](int a, int b) {
        return fabs(eigenvalues[a] - shift) < fabs(eigenvalues[b] - shift);
    });

    VectorXd sortedVals(n);
    for (int i = 0; i < n; i++) {
        sortedVals[i] = eigenvalues[indices[i]];
    }
    MatrixXd sortedVecs(eigenvectors.rows(), n);
    for (int i = 0; i < n; i++) {
        sortedVecs.col(i) = eigenvectors.col(indices[i]);
    }
...
//=============================================================================
vector<double> davidsonShifted(Hmult2& H, vector<MatrixXx>& x0, MatrixXx& diag, int maxCopies, double tol, int& numIter, bool print, double shift) {
//-----------------------------------------------------------------------------
    /*!
    BM_description
...
//-----------------------------------------------------------------------------
  ...
  while(true) {
    ...
    if (commrank == 0) {
      ...
      sortEigenpairs(eigenvalues, eigenvectors, shift);

      b.block(0,0,b.rows(), bsize) = b.block(0,0,b.rows(), bsize)*eigenvectors;
      sigma.block(0,0,b.rows(), bsize) = sigma.block(0,0,b.rows(), bsize)*eigenvectors;

      ei = eigenvalues[convergedRoot];
      for (int i=0; i<convergedRoot; i++) {
        r = sigma.col(i) - eigenvalues[i]*b.col(i);
        double error = r.norm();
        if (error > tol) {
          convergedRoot = i;
      ...
      if (hsubspace.rows() == b.rows()) {
        ...
        for (int i=0; i<x0.size(); i++) {
          x0[i] = b.col(i);
          eroots.push_back(eigenvalues[i]);
          if (print ) pout << str(boost::format("#niter:%3d root:%3d -> Energy : %18.10g  \n") %(numIter) % (i) % eroots[i] );
        }
        continueOrReturn = 2;
        goto label1;
